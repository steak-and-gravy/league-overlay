...
class leagueOverlay:
    ...
    def setup_gui(self):
        ...
        self.division_filter_btn = tk.Button(self.button_frame, text="My Divison", command=self.toggle_division_filter,
    ...
    def create_headers(self):
        """Create column headers in the fixed header frame"""
        # Clear existing headers
        for widget in self.header_frame.winfo_children():
            widget.destroy()
        self.root.update_idletasks()
        headers = [
            ("Pos", round(self.root.winfo_width() * .11)), # default 40 at 375 width
            ("D-Pos", round(self.root.winfo_width() * .11)), # default 40 at 375 width
            ("Car#", round(self.root.winfo_width() * .13)), # default 50 at 375 width
            ("Driver", round(self.root.winfo_width() * .44)), # default 165 at 375 width
            ("Div Gap", round(self.root.winfo_width() * .2)) # default 75 at 375 width
        ]
        
        for header, width in headers:
            label = tk.Label(self.header_frame, text=header, fg='white', bg='#333333',
                           font=('Arial', 9, 'bold'), width=width//8)
            label.pack(side=tk.LEFT, padx=2)

    def toggle_division_filter(self):
        """Toggle showing only player's division"""
        self.show_only_my_division = not self.show_only_my_division
        button_text = "All Divisions" if self.show_only_my_division else "My Division"
        button_color = '#4CAF50' if self.show_only_my_division else '#555555'
    ...
    def display_race_data(self):
        ...
        # Filter data if division filter is enabled
    ...
    def update_existing_display(self, data):
        ...
        for i, driver_data in enumerate(data):
            ...
            if car_idx in self.data_widgets:
                ...
                if widgets['name']['text'] != driver_data['driver_name'][:20]:
